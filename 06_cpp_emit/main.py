...
def emit_param_conversion(p: dict, mi: int, lines: list,
    ...
    # [A37] jchar array — UTF-16 u16string
        ...
        lines += [
            ...
            f"{indent}        // [PATCH-2A] UTF-8→UTF-16→NewString",
            f"{indent}        std::string _sa = nb::cast<std::string>(_item);",
            f"{indent}        const uint8_t* _sub = reinterpret_cast<const uint8_t*>(_sa.data());",
            f"{indent}        size_t _sul = _sa.size(); std::u16string _su16; _su16.reserve(_sul);",
            f"{indent}        for (size_t _sui = 0; _sui < _sul; ) {{",
            f"{indent}            uint32_t _scp; uint8_t _sb0 = _sub[_sui];",
            f"{indent}            if (_sb0 < 0x80u) {{ _scp = _sb0; _sui += 1; }}",
            f"{indent}            else if ((_sb0&0xE0u)==0xC0u && _sui+1<_sul) {{ _scp=((uint32_t)(_sb0&0x1Fu)<<6)|(_sub[_sui+1]&0x3Fu); _sui+=2; }}",
            f"{indent}            else if ((_sb0&0xF0u)==0xE0u && _sui+2<_sul) {{ _scp=((uint32_t)(_sb0&0x0Fu)<<12)|((uint32_t)(_sub[_sui+1]&0x3Fu)<<6)|(_sub[_sui+2]&0x3Fu); _sui+=3; }}",
            f"{indent}            else if ((_sb0&0xF8u)==0xF0u && _sui+3<_sul) {{ _scp=((uint32_t)(_sb0&0x07u)<<18)|((uint32_t)(_sub[_sui+1]&0x3Fu)<<12)|((uint32_t)(_sub[_sui+2]&0x3Fu)<<6)|(_sub[_sui+3]&0x3Fu); _sui+=4; }}",
            f"{indent}            else {{ ++_sui; continue; }}",
            f"{indent}            if (_scp < 0x10000u) {{ _su16 += (char16_t)_scp; }}",
            f"{indent}            else if (_scp < 0x110000u) {{ _scp -= 0x10000u; _su16 += (char16_t)(0xD800u|(_scp>>10)); _su16 += (char16_t)(0xDC00u|(_scp&0x3FFu)); }}",
            f"{indent}        }}",
            f"{indent}        _jitem = env->NewString(reinterpret_cast<const jchar*>(_su16.data()), (jsize)_su16.size());",
            f"{indent}    }}",
    ...
    for f in fields:
        ...
        if is_static:
            ...
            if cpp_t == "std::string":
                lines += [
                    ...
                    f"    if (!_r) return \"\";",
                    f"    // [PATCH-1E] UTF-16→UTF-8 for field getter",
                    f"    jsize _fgl = env->GetStringLength(_r);",
                    f"    const jchar* _fgc = env->GetStringChars(_r, nullptr);",
                    f"    std::string _s;",
                    f"    if (_fgc && _fgl > 0) {{",
                    f"        _s.reserve((size_t)_fgl * 3);",
                    f"        for (jsize _fgi = 0; _fgi < _fgl; ) {{",
                    f"            uint32_t _fgcp; uint16_t _fgc1 = (uint16_t)_fgc[_fgi++];",
                    f"            if (_fgc1 >= 0xD800u && _fgc1 <= 0xDBFFu && _fgi < _fgl) {{",
                    f"                uint16_t _fgc2 = (uint16_t)_fgc[_fgi];",
                    f"                if (_fgc2 >= 0xDC00u && _fgc2 <= 0xDFFFu) {{ _fgcp = 0x10000u+(((uint32_t)(_fgc1-0xD800u))<<10)+(uint32_t)(_fgc2-0xDC00u); ++_fgi; }}",
                    f"                else {{ _fgcp = _fgc1; }}",
                    f"            }} else {{ _fgcp = _fgc1; }}",
                    f"            if (_fgcp < 0x80u) {{ _s += (char)_fgcp; }}",
                    f"            else if (_fgcp < 0x800u) {{ _s += (char)(0xC0u|(_fgcp>>6)); _s += (char)(0x80u|(_fgcp&0x3Fu)); }}",
                    f"            else if (_fgcp < 0x10000u) {{ _s += (char)(0xE0u|(_fgcp>>12)); _s += (char)(0x80u|((_fgcp>>6)&0x3Fu)); _s += (char)(0x80u|(_fgcp&0x3Fu)); }}",
                    f"            else {{ _s += (char)(0xF0u|(_fgcp>>18)); _s += (char)(0x80u|((_fgcp>>12)&0x3Fu)); _s += (char)(0x80u|((_fgcp>>6)&0x3Fu)); _s += (char)(0x80u|(_fgcp&0x3Fu)); }}",
                    f"        }}",
                    f"    }}",
                    f"    if (_fgc) env->ReleaseStringChars(_r, _fgc);",
            ...
            if cpp_t == "std::string":
                lines += [
                    ...
                    f"    if (!_r) return \"\";",
                    f"    // [PATCH-1E] UTF-16→UTF-8 for instance field getter",
                    f"    jsize _fgl = env->GetStringLength(_r);",
                    f"    const jchar* _fgc = env->GetStringChars(_r, nullptr);",
                    f"    std::string _s;",
                    f"    if (_fgc && _fgl > 0) {{",
        ...
        if setter_fn:
            if is_static:
                ...
                if cpp_t == "std::string":
                    lines += [
                        f"    // [PATCH-2C] UTF-8→UTF-16→NewString for field setter",
                        f"    jstring _s;",
                        f"    {{",
                        f"        const uint8_t* _fsub = reinterpret_cast<const uint8_t*>(val.data());",
                        f"        size_t _fsul = val.size(); std::u16string _fsu16; _fsu16.reserve(_fsul);",
                        f"        for (size_t _fsui = 0; _fsui < _fsul; ) {{",
                        f"            uint32_t _fscp; uint8_t _fsb0 = _fsub[_fsui];",
                        f"            if (_fsb0 < 0x80u) {{ _fscp = _fsb0; _fsui += 1; }}",
                        f"            else if ((_fsb0&0xE0u)==0xC0u && _fsui+1<_fsul) {{ _fscp=((uint32_t)(_fsb0&0x1Fu)<<6)|(_fsub[_fsui+1]&0x3Fu); _fsui+=2; }}",
                        f"            else if ((_fsb0&0xF0u)==0xE0u && _fsui+2<_fsul) {{ _fscp=((uint32_t)(_fsb0&0x0Fu)<<12)|((uint32_t)(_fsub[_fsui+1]&0x3Fu)<<6)|(_fsub[_fsui+2]&0x3Fu); _fsui+=3; }}",
                        f"            else if ((_fsb0&0xF8u)==0xF0u && _fsui+3<_fsul) {{ _fscp=((uint32_t)(_fsb0&0x07u)<<18)|((uint32_t)(_fsub[_fsui+1]&0x3Fu)<<12)|((uint32_t)(_fsub[_fsui+2]&0x3Fu)<<6)|(_fsub[_fsui+3]&0x3Fu); _fsui+=4; }}",
                        f"            else {{ ++_fsui; continue; }}",
                        f"            if (_fscp < 0x10000u) {{ _fsu16 += (char16_t)_fscp; }}",
                        f"            else if (_fscp < 0x110000u) {{ _fscp -= 0x10000u; _fsu16 += (char16_t)(0xD800u|(_fscp>>10)); _fsu16 += (char16_t)(0xDC00u|(_fscp&0x3FFu)); }}",
                        f"        }}",
                        f"        _s = env->NewString(reinterpret_cast<const jchar*>(_fsu16.data()), (jsize)_fsu16.size());",
                        f"    }}",
                ...
                if cpp_t == "std::string":
                    lines += [
                        f"    // [PATCH-2C] UTF-8→UTF-16→NewString for instance field setter",
                        f"    jstring _s;",
                        f"    {{",
                        f"        const uint8_t* _fsub = reinterpret_cast<const uint8_t*>(val.data());",
                        f"        size_t _fsul = val.size(); std::u16string _fsu16; _fsu16.reserve(_fsul);",
    ...
    auto call_fn = [&]() -> jobject {
        try {
                ...
                for (jsize i = 0; i < len; ++i) {
                    ...
                    if (!unboxed) {
                        if (g_jstring_class
                            ...
                            // [PATCH-1D] UTF-16→UTF-8 for dispatch string args
                            jsize _dsl = env->GetStringLength((jstring)elem);
                            const jchar* _dsc = env->GetStringChars((jstring)elem, nullptr);
                            std::string _dsr;
                            if (_dsc && _dsl > 0) {
                                _dsr.reserve((size_t)_dsl * 3);
                                for (jsize _dsi = 0; _dsi < _dsl; ) {
                                    uint32_t _dcp; uint16_t _dc1 = (uint16_t)_dsc[_dsi++];
                                    if (_dc1 >= 0xD800u && _dc1 <= 0xDBFFu && _dsi < _dsl) {
                                        uint16_t _dc2 = (uint16_t)_dsc[_dsi];
                                        if (_dc2 >= 0xDC00u && _dc2 <= 0xDFFFu) { _dcp = 0x10000u+(((uint32_t)(_dc1-0xD800u))<<10)+(uint32_t)(_dc2-0xDC00u); ++_dsi; }
                                        else { _dcp = _dc1; }
                                    } else { _dcp = _dc1; }
                                    if (_dcp < 0x80u) { _dsr += (char)_dcp; }
                                    else if (_dcp < 0x800u) { _dsr += (char)(0xC0u|(_dcp>>6)); _dsr += (char)(0x80u|(_dcp&0x3Fu)); }
                                    else if (_dcp < 0x10000u) { _dsr += (char)(0xE0u|(_dcp>>12)); _dsr += (char)(0x80u|((_dcp>>6)&0x3Fu)); _dsr += (char)(0x80u|(_dcp&0x3Fu)); }
                                    else { _dsr += (char)(0xF0u|(_dcp>>18)); _dsr += (char)(0x80u|((_dcp>>12)&0x3Fu)); _dsr += (char)(0x80u|((_dcp>>6)&0x3Fu)); _dsr += (char)(0x80u|(_dcp&0x3Fu)); }
                                }
                            }
                            if (_dsc) env->ReleaseStringChars((jstring)elem, _dsc);
                            py_args.append(nb::str(_dsr.c_str()));
                            LOGV("DISPATCH_ARG[%d] = str (utf16)", (int)i);
            ...
            if (nb::isinstance<nb::str>(py_result)) {
                // [PATCH-2D] UTF-8→UTF-16→NewString
                std::string _rs = nb::cast<std::string>(py_result);
                const uint8_t* _rub = reinterpret_cast<const uint8_t*>(_rs.data());
                size_t _rul = _rs.size(); std::u16string _ru16; _ru16.reserve(_rul);
                for (size_t _rui = 0; _rui < _rul; ) {
                    uint32_t _rcp; uint8_t _rb0 = _rub[_rui];
                    if (_rb0 < 0x80u) { _rcp = _rb0; _rui += 1; }
                    else if ((_rb0&0xE0u)==0xC0u && _rui+1<_rul) { _rcp=((uint32_t)(_rb0&0x1Fu)<<6)|(_rub[_rui+1]&0x3Fu); _rui+=2; }
                    else if ((_rb0&0xF0u)==0xE0u && _rui+2<_rul) { _rcp=((uint32_t)(_rb0&0x0Fu)<<12)|((uint32_t)(_rub[_rui+1]&0x3Fu)<<6)|(_rub[_rui+2]&0x3Fu); _rui+=3; }
                    else if ((_rb0&0xF8u)==0xF0u && _rui+3<_rul) { _rcp=((uint32_t)(_rb0&0x07u)<<18)|((uint32_t)(_rub[_rui+1]&0x3Fu)<<12)|((uint32_t)(_rub[_rui+2]&0x3Fu)<<6)|(_rub[_rui+3]&0x3Fu); _rui+=4; }
                    else { ++_rui; continue; }
                    if (_rcp < 0x10000u) { _ru16 += (char16_t)_rcp; }
                    else if (_rcp < 0x110000u) { _rcp -= 0x10000u; _ru16 += (char16_t)(0xD800u|(_rcp>>10)); _ru16 += (char16_t)(0xDC00u|(_rcp&0x3FFu)); }
                }
                return env->NewString(reinterpret_cast<const jchar*>(_ru16.data()), (jsize)_ru16.size());
            }
