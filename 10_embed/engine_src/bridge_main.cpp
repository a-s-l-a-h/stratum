...
// Generated further down by the NB_MODULE(stratum, m) macro. Forward-declared
// here because JNI_OnLoad (which appears earlier in this file) calls it
// manually: System.loadLibrary() dlopens us directly, bypassing Python's
// file-based import discovery, so nothing else would ever trigger this
...
extern "C" JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void*) {
    ...
    // IMPORTANT: nanobind builds extension modules using PEP 489 multi-phase
    // initialization. Calling PyInit_stratum() directly and treating its
    // return value as a ready-to-use module is WRONG for a multi-phase
    // module — it actually returns a PyModuleDef-wrapping object, not a real
    // module instance. PyModule_GetDict() on that object fails with
    // "SystemError: bad argument to internal function" and returns NULL,
    // and evaluating code with NULL globals segfaults inside PyEval_EvalCode.
    //
    // The correct way to finish multi-phase init is to let CPython's own
    // import machinery do it. "stratum" was already registered in the
    // inittab (see native-lib.cpp's register_stratum_inittab(), called
    // BEFORE Py_Initialize()), so a normal PyImport_ImportModule("stratum")
    // here performs the full import sequence correctly and returns a real,
    // valid module object — exactly as if Python code had written
    // `import stratum` itself. It also inserts "stratum" into sys.modules
    // for us, so no manual PyDict_SetItemString is needed.
    {
        PyGILState_STATE gstate = PyGILState_Ensure();
        PyObject* mod = PyImport_ImportModule("stratum");
        if (mod) {
            PyObject* g = PyModule_GetDict(mod);  // safe: mod is a REAL module now

            // Exec the bootstrap (installs the meta-path finder that lazily
            // serves every generated class's compressed source), then the
            // __init__ body directly into this module's own namespace — so
            // getActivity()/to_java()/@export land as stratum.getActivity() etc.
            ...
            Py_DECREF(mod);  // sys.modules already holds its own reference

            LOGI("Stratum: embedded 'stratum' package imported and booted.");
        } else {
            PyErr_Print();
            LOGE("Stratum: import stratum failed -- was PyInit_stratum registered "
                 "in the inittab before Py_Initialize()?");
        }
...
// Passthrough helper exposed to the embedded bootstrap loader.
...
NB_MODULE(stratum, m) {
    ...
    m.attr("__package__") = "stratum";   // makes it safe as a relative-import anchor too
